    ...
    try:
        with open(file_path, 'r', encoding=encoding) as file:
            ...
            if not content.strip():
                # Empty file - likely being written to, wait and retry
                import time
                time.sleep(0.1)
                file.seek(0)
                content = file.read()
                if not content.strip():
                    # Still empty after retry - raise error for caller to handle
                    raise json.JSONDecodeError("File is empty after retry", file_path, 0)
            file.seek(0)  # Reset file pointer for json.load
