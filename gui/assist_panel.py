...
class AssistPanel:
    ...
    def _create_assist_modes_block(self, parent):
        ...
        # Generate assist mode labels with current hotkeys
        hotkey_mapping = {
            'FSD Route Assist': self.ed_ap.config.get('HotKey_StartFSD', 'ins').upper(),
            'Supercruise Assist': self.ed_ap.config.get('HotKey_StartSC', 'home').upper(),
            'Waypoint Assist': self.ed_ap.config.get('HotKey_StartWaypoint', 'del').upper(),
            'Robigo Assist': self.ed_ap.config.get('HotKey_StartRobigo', 'pgup').upper(),
            'AFK Combat Assist': None,  # No hotkey
            'DSS Assist': None  # No hotkey
        }
        
        # Store base mode names for internal use
        self.base_modes = ('FSD Route Assist', 'Supercruise Assist', 'Waypoint Assist', 
                          'Robigo Assist', 'AFK Combat Assist', 'DSS Assist')
        
        modes_check_fields = []
        for mode, hotkey in hotkey_mapping.items():
            if hotkey:
                modes_check_fields.append(f"{mode} ({hotkey})")
            else:
                modes_check_fields.append(mode)
        modes_check_fields = tuple(modes_check_fields)
        ...
        for field in modes_check_fields:
            ...
            # Add tooltip - use base mode name for tooltip lookup
            base_field = field.split(' (')[0]  # Remove hotkey part from field name
            tip = Hovertip(row, self.tooltips[base_field], hover_delay=1000)
    
    def refresh_hotkey_labels(self):
        """Refresh checkbox labels with current hotkeys"""
        hotkey_mapping = {
            'FSD Route Assist': self.ed_ap.config.get('HotKey_StartFSD', 'ins').upper(),
            'Supercruise Assist': self.ed_ap.config.get('HotKey_StartSC', 'home').upper(),
            'Waypoint Assist': self.ed_ap.config.get('HotKey_StartWaypoint', 'del').upper(),
            'Robigo Assist': self.ed_ap.config.get('HotKey_StartRobigo', 'pgup').upper(),
            'AFK Combat Assist': None,
            'DSS Assist': None
        }
        
        # Update checkbox text labels
        for i, base_mode in enumerate(self.base_modes):
            hotkey = hotkey_mapping[base_mode]
            if hotkey:
                new_label = f"{base_mode} ({hotkey})"
            else:
                new_label = base_mode
            
            # Find the checkbox and update its text
            if hasattr(self, 'lab_ck'):
                for field, checkbox in self.lab_ck.items():
                    if field.startswith(base_mode):
                        checkbox.config(text=new_label)
                        # Update the key in lab_ck dict if needed
                        if field != new_label:
                            self.lab_ck[new_label] = self.lab_ck.pop(field)
                            # Also update checkboxvar dict
                            if hasattr(self, 'checkboxvar') and field in self.checkboxvar:
                                self.checkboxvar[new_label] = self.checkboxvar.pop(field)
                        break
    ...
    def _emergency_stop_all(self):
        ...
        # Re-enable all assist checkboxes since emergency stop should reset everything
        for cb_field in ['FSD Route Assist (INS)', 'Supercruise Assist (HOME)', 'Waypoint Assist (DEL)', 
                        'Robigo Assist (PG UP)', 'AFK Combat Assist', 'DSS Assist']:
            if cb_field in self.lab_ck:
                self.lab_ck[cb_field].config(state='active')
